...
class LoginPage extends Component {
  ...
  handleSubmit = async (e) => {
    ...
    if (login.trim().length === 0 ) return console.error('Login inválido');
    if (senha.trim().length < 6) return console.error('Senha inválida');
    ...
    if (!resposta.ok) {
      this.setState({ mensagemErro: 'Algo de errado aconteceu' });
      return;
    }
